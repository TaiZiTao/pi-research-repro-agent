...
> 注记（2026-09-04 起步）：已新增无第三方依赖的轨迹 Schema、递归路径/Token 脱敏、工具调用配对与未知工具校验；依据真实复现 smoke 记录首条 `reproduction_error_repair` 黄金轨迹，并生成通过校验的 `training/data/raw/golden.jsonl`。自动采集和批量扩展尚未开始。
