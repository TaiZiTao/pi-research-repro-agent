...
export async function runNpx(args: string[], opts: RunNpxOptions = {}): Promise<RunNpxResult> {
  // Prefer Electron's Node runtime when embedded (no user PATH dependency).
  // ELECTRON_RUN_AS_NODE makes process.execPath behave as node.
  ...
  return execFileAsync(command, commandArgs, {
    ...
    cwd: opts.cwd,
    env: { ...opts.env, ...(process.versions.electron ? { ELECTRON_RUN_AS_NODE: "1" } : {}) },
  });
}
