...
export class BrowserHeaderRuleStore {
  ...
  set(profileId: string, direction: "request" | "response", rules: BrowserHeaderRule[]): void {
    this.value[direction][profileId] = structuredClone(rules);
    this.persist();
  }
  ...
  private read(): RuleFile {
    try {
      ...
      const parsed = JSON.parse(fs.readFileSync(this.filePath, "utf8")) as Partial<RuleFile>;
      if (parsed.version !== VERSION || !isRecord(parsed.request) || !isRecord(parsed.response)) return emptyFile();
      return {
        version: VERSION,
