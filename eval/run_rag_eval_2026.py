import json, os, subprocess, glob, pathlib
...
def cli(*args):
    ...
    if p.returncode != 0:
        raise RuntimeError((p.stderr or p.stdout or "")[:500])
    return json.loads(p.stdout)
def find_pdf(key):
    for f in glob.glob(os.path.join(PAPERS, "*.pdf")):
        if key.lower() in os.path.basename(f).lower():
            return f
    raise RuntimeError("pdf not found for " + key)
ann = json.load(open(ANN, encoding="utf-8"))["items"]
keys = sorted({a["paper_key"] for a in ann})
proj = {}
for key in keys:
    pdf = find_pdf(key)
    r = cli("import", "--pdf", pdf, "--title", pathlib.Path(pdf).stem[:80], "--user-data", USER)
    pid = r["project"]["projectId"]
    proj[key] = pid
    print("imported", key, pid, r["project"]["status"], flush=True)
rows = []
for a in ann:
    r = cli("search", "--project", proj[a["paper_key"]], "--query", a["query"], "--limit", "5", "--user-data", USER)
    pages = [h["page"] for h in r["hits"]]
    gold = a["gold_pages"][0]
    rank = next((i + 1 for i, p in enumerate(pages) if p == gold), None)
    rows.append({**a, "returned_pages": pages, "hit_rank": rank})
    print("q", a["id"], "rank", rank, flush=True)
n = len(rows)
def rate(f): return sum(1 for r in rows if f(r)) / n
hit1 = rate(lambda r: r["hit_rank"] == 1)
hit3 = rate(lambda r: r["hit_rank"] is not None and r["hit_rank"] <= 3)
hit5 = rate(lambda r: r["hit_rank"] is not None)
...
with open(os.path.join(ROOT, "eval", "rag-eval-2026.json"), "w", encoding="utf-8") as fh:
