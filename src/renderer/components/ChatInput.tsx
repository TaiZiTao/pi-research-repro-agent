...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const toolPresetKey = Object.entries(TOOL_PRESET_MAP).find(([, value]) => value === (toolPreset ?? "default"))?.[0] as "off" | "default" | "full" | undefined ?? "default";
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{
                ...
                {t("queued", "Queued")} · {(queuedMessages?.steering.length ?? 0) + (queuedMessages?.followUp.length ?? 0)}
              </span>
              {onRecallQueue && (
                <button
                  onClick={onRecallQueue}
                  title={t("recallQueueDescription", "Remove all queued messages and put them back into the input box for editing")}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: 6,
        ...
        {retryInfo && (
          <div style={{
            ...
            {t("retrying", "Retrying")} ({retryInfo.attempt}/{retryInfo.maxAttempts})…{retryInfo.errorMessage && <span style={{ opacity: 0.7, marginLeft: 4 }}>— {retryInfo.errorMessage}</span>}
          </div>
        )}
        {compactResultText && (
          <div style={{
          ...
          <textarea
            ...
            placeholder={
              isStreaming && (onSteer || onFollowUp)
                ...
                : isStreaming ? t("agentRunning", "Agent is running…")
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0, alignSelf: "flex-end" }}>
              {onSteer && (
                <button
                  ...
                  title={attachedImages.length ? t("imageQueueUnavailable", "Image attachments cannot be queued while the agent is running") : t("steerDescription", "Interrupt the current run and inject this message now")}
                  style={{
              ...
              {onFollowUp && (
                <button
                  ...
                  disabled={!canQueueStreamingMessage}
                  title={attachedImages.length ? t("imageQueueUnavailable", "Image attachments cannot be queued while the agent is running") : t("followUpDescription", "Queue this message after the agent finishes")}
                  style={{
                    display: "flex", alignItems: "center", gap: 5,
                    padding: "7px 12px",
          ...
          <div ref={controlsMenuRef} style={{
                    ...
                    {THINKING_LEVELS.filter((lvl) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span style={{ flex: 1 }}>
                            {displayLabel}
                            {showOriginal && <span style={{ fontSize: 10, color: "var(--text-dim)", fontFamily: "var(--font-mono)", marginLeft: 5 }}>({thinkingLabels[lvl]})</span>}
                          </span>
            ...
            {onCompact && (!isStreaming || isCompacting) && (
              <div style={{ position: "relative" }}>
                ...
                >
                  {isCompacting ? (
                    <><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><rect x="2" y="2" width="6" height="6" rx="1" fill="currentColor" /></svg>{(!isMobile || controlsMenuOpen) && <span style={{ whiteSpace: "nowrap" }}>{t("compacting", "Compacting…")}</span>}</>
                  ) : (
                    <><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="4 14 10 14 10 20" /><polyline points="20 10 14 10 14 4" />
                      <line x1="10" y1="14" x2="3" y2="21" /><line x1="21" y1="3" x2="14" y2="10" />
                    </svg>{(!isMobile || controlsMenuOpen) && <span style={{ whiteSpace: "nowrap" }}>{t("compact", "Compact")}</span>}</>
                  )}
                </button>
              </div>
            )}
            ...
            {onSoundToggle !== undefined && (
              <button
                ...
                title={soundEnabled ? t("disableCompletionSound", "Disable completion sound") : t("enableCompletionSound", "Enable completion sound")}
                aria-label={soundEnabled ? t("disableCompletionSound", "Disable completion sound") : t("enableCompletionSound", "Enable completion sound")}
                style={{
                  display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
