...
export function checkRendererI18n({ root, rendererRoot, dictionariesPath }) {
  ...
  for (const file of rendererSourceFiles(rendererRoot, dictionariesPath)) {
    ...
    if (file.includes(`${path.sep}components${path.sep}browser${path.sep}`)) {
      checkBrowserVisibleLiterals({ failures, root, file, sourceText, source });
    }
  }
...
function checkBrowserVisibleLiterals({ failures, root, file, sourceText, source }) {
  walk(source, (node) => {
